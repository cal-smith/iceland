...
var ImageList = function (_HTMLComponent) {
	_inherits(ImageList, _HTMLComponent);

	function ImageList() {
		_classCallCheck(this, ImageList);

		var _this = _possibleConstructorReturn(this, (ImageList.__proto__ || Object.getPrototypeOf(ImageList)).call(this));
	...
	_createClass(ImageList, [{
		key: 'resetArrows',
		value: function resetArrows(previous, next) {
		...
		value: function connectedCallback() {
			...
			this.images.forEach(function (image) {
				return image.removeAttribute("height");
			});
			this.shadow.innerHTML = '\n\t\t\t<div class="carousel">\n\t\t\t\t<button class="previous" disabled></button>\n\t\t\t\t<div class="current-image">\n\t\t\t\t\t' + this.images[this.index].outerHTML + '\n\t\t\t\t</div>\n\t\t\t\t<button class="next"></button>\n\t\t\t\t<div class="imglist">\n\t\t\t\t\t' + this.images.reduce(function (acc, img, index) {
				return acc += '<div class="img" data-index="' + index + '">' + img.outerHTML + '</div>';
			}, "") + '\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t\t<style>' + this.styles + '</style>\n\t\t';
			var currentImage = this.shadow.querySelector(".current-image");
			var previous = this.shadow.querySelector(".previous");
			var next = this.shadow.querySelector(".next");
			this.shadow.querySelectorAll(".img").forEach(function (img) {
				var self = _this2;
			...
			previous.addEventListener("click", function () {
				if (_this2.index > 0) {
					_this2.index--;
					currentImage.innerHTML = _this2.images[_this2.index].outerHTML;
					_this2.resetArrows(previous, next);
				}
			});
			next.addEventListener("click", function () {
				if (_this2.index < _this2.images.length - 1) {
					_this2.index++;
					currentImage.innerHTML = _this2.images[_this2.index].outerHTML;
					_this2.resetArrows(previous, next);
				}
